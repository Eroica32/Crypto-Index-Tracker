...
allSymbols = API.getSymbol(API.cmc)
...
coreData = API.extractPriceAndMarketCap(coreData)
...
coreData['percentage till index'] = 1 - coreData['price'] * list(targetCoin['myCoins']) / coreData['how much to buy']
coreData.sort_values(by='percentage till index', ascending=True)
coreData[['name', 'how much to buy', 'how much till index', 'percentage till index']].to_csv('CryptoIndexSummary.csv')
